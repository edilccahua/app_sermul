<template>
  ...
  >
    ...
    >
      <div class="flex items-center gap-3 mb-2">
        <button
          ...
          @click="$router.push('/app/reservas')"
        >
        ...
        <div v-if="reserva">
          ...
          >
            Despachar Reserva
            <code
              class="font-mono text-base ml-2"
              :style="{ color: 'var(--sapButton_Emphasized_Background)' }"
            >{{ reserva.codigo_reserva }}</code>
          </h1>
          <p
            class="text-sm"
            :style="{ color: 'var(--sapContent_LabelColor)' }"
          >
            {{ reserva.grupo?.nombre }} · {{ reserva.parada ? reserva.parada.codigo + ' - ' + reserva.parada.nombre : '' }} · Turno {{ reserva.turno }}
          </p>
        </div>
        <div
          v-if="reserva"
        ...
        >
          <span
            ...
            :style="estadoBadgeStyle(reserva.estado)"
          >
            {{ reserva.estado }}
          </span>
        </div>
    ...
    >
      <div
        class="p-6 rounded-sm border text-center"
    ...
    >
      <div class="max-w-3xl mx-auto space-y-4">
        ...
        >
          ...
          <!-- Filas de ítems -->
          <div
            v-for="item in reserva.detalles ?? []"
            :key="item.id"
          ...
          >
            ...
            <div class="flex-1 min-w-0">
              ...
              <code
                ...
                :style="{ color: 'var(--sapContent_LabelColor)' }"
              >{{ item.catalogo?.codigo_interno }}</code>
            </div>
            <span
              class="w-24 text-sm text-center"
              :style="{ color: 'var(--sapTextColor)' }"
            >{{ item.cantidad }}</span>
        ...
        <div class="flex gap-3">
          <button
            class="flex-1 h-12 text-sm border rounded-sm"
          ...
          >
            ...
            ><span class="sap-icon--refresh animate-spin" /> Despachando...</span>
