  ...
  <div class="h-full flex flex-col" :style="{ background: 'var(--sapBackgroundColor)' }">
    ...
    <div class="px-6 py-4 border-b shrink-0" :style="{ background: 'var(--sapPageHeader_Background)', borderColor: 'var(--sapList_BorderColor)' }">
      <div class="flex items-center gap-3">
        <router-link to="/app/grupos" class="h-8 w-8 flex items-center justify-center rounded-sm hover:bg-muted/30 transition-colors"
          :style="{ color: 'var(--sapContent_LabelColor)' }">
          <span class="sap-icon--nav-back" />
        </router-link>
        <span class="sap-icon--excel-attachment text-2xl" :style="{ color: 'var(--sapButton_Emphasized_Background)' }" />
        <div>
          <h1 class="text-xl font-bold" :style="{ color: 'var(--sapTextColor)' }">Importar Grupos desde Excel</h1>
    ...
    <div class="flex-1 overflow-y-auto p-6">
      <div class="max-w-2xl mx-auto space-y-4">
        ...
        <div class="p-6 rounded-sm border" :style="{ background: 'var(--sapGroup_ContentBackground)', borderColor: 'var(--sapList_BorderColor)' }">
          <h2 class="text-base font-semibold mb-4" :style="{ color: 'var(--sapTextColor)' }">Configuración</h2>
          <div class="space-y-1.5">
            <label class="text-sm font-normal" :style="{ color: 'var(--sapContent_LabelColor)' }">Parada de destino *:</label>
            <select v-model="paradaId" class="h-10 w-full border px-3 text-sm rounded-sm" :style="selectStyle">
        ...
        <div class="p-4 rounded-sm border" :style="{ background: 'var(--sapInformationBackground)', borderColor: 'var(--sapInformationBorderColor)' }">
          <div class="flex items-start gap-2">
            <span class="sap-icon--message-information shrink-0 mt-0.5" :style="{ color: 'var(--sapInformationTextColor)' }" />
            <div class="flex-1">
              <p class="text-sm font-semibold mb-1" :style="{ color: 'var(--sapTextColor)' }">Columnas esperadas en el Excel:</p>
              <div class="flex flex-wrap gap-1">
                <code
                  v-for="col in columnasEsperadas" :key="col"
              ...
              <p class="text-xs mt-2" :style="{ color: 'var(--sapContent_LabelColor)' }">
        ...
        >
          ...
          />
          <p class="text-sm font-medium mb-1" :style="{ color: 'var(--sapTextColor)' }">
        ...
        <div v-if="resultado" class="space-y-4">
          ...
          <div class="grid grid-cols-2 sm:grid-cols-4 gap-3">
            <div v-for="stat in resumenStats" :key="stat.label"
              ...
              <p class="text-xs mt-1" :style="{ color: 'var(--sapContent_LabelColor)' }">{{ stat.label }}</p>
            </div>
          </div>

          <!-- Grupos creados -->
          <div v-if="resultado.grupos_creados?.length" class="p-4 rounded-sm border"
            :style="{ background: 'var(--sapSuccessBackground)', borderColor: 'var(--sapSuccessBorderColor)' }">
            <p class="text-sm font-semibold mb-2" :style="{ color: 'var(--sapTextColor)' }">Grupos creados:</p>
            <div class="flex flex-wrap gap-2">
              <span v-for="g in resultado.grupos_creados" :key="g.codigo"
                ...
                <code class="font-mono">{{ g.codigo }}</code> — {{ g.nombre }} ({{ g.integrantes_count }} integrantes)
              </span>
            </div>
          </div>

          <!-- Tabla de errores -->
          <div v-if="resultado.errores?.length" class="rounded-sm border overflow-hidden"
            ...
            <div class="px-4 py-2 border-b flex items-center gap-2"
              ...
              <p class="text-xs font-semibold" :style="{ color: 'var(--sapNegativeTextColor)' }">
                {{ resultado.errores.length }} error{{ resultado.errores.length > 1 ? 'es' : '' }}
              </p>
            </div>
            <div v-for="(err, i) in resultado.errores" :key="i"
