  ...
  <div class="h-full flex flex-col" :style="{ background: 'var(--sapBackgroundColor)' }">
    ...
    <div class="px-6 py-4 border-b shrink-0" :style="{ background: 'var(--sapPageHeader_Background)', borderColor: 'var(--sapList_BorderColor)' }">
      <div class="flex items-center gap-3 mb-4">
        <span class="sap-icon--list text-2xl" :style="{ color: 'var(--sapButton_Emphasized_Background)' }" />
        <div>
          <h1 class="text-xl font-bold" :style="{ color: 'var(--sapTextColor)' }">Reservas</h1>
      ...
      <div class="flex items-center gap-3 flex-wrap">
        <div class="flex items-center gap-2">
          <label class="text-xs" :style="{ color: 'var(--sapContent_LabelColor)' }">Estado:</label>
          <select v-model="filtros.estado" class="h-8 border px-2 text-xs rounded-sm" :style="selectStyle">
        ...
        <div class="flex items-center gap-2">
          <label class="text-xs" :style="{ color: 'var(--sapContent_LabelColor)' }">Parada:</label>
          <select v-model="filtros.parada_id" class="h-8 border px-2 text-xs rounded-sm" :style="selectStyle">
    ...
    <!-- Table Toolbar -->
    <div class="flex items-center gap-3 px-6 py-2 border-b shrink-0"
    ...
    <div class="flex-1 overflow-auto">
      <div v-if="reservasStore.loading" class="flex items-center justify-center py-8">
      ...
      <table v-else class="w-full text-xs border-collapse">
        <thead>
          <tr :style="{ background: 'var(--sapList_HeaderBackground)', color: 'var(--sapContent_LabelColor)' }">
            <th v-for="col in columnas" :key="col" class="px-4 py-1.5 text-left font-semibold uppercase tracking-wide border-b" :style="{ borderColor: 'var(--sapList_BorderColor)' }">{{ col }}</th>
          </tr>
        </thead>
        <tbody>
          <tr
          ...
          >
            <td class="px-4 py-1.5">
              <code class="font-mono" :style="{ color: 'var(--sapButton_Emphasized_Background)' }">{{ r.codigo }}</code>
            </td>
            <td class="px-4 py-1.5">{{ r.grupo?.nombre ?? '—' }}</td>
            <td class="px-4 py-1.5" :style="{ color: 'var(--sapContent_LabelColor)' }">{{ r.parada?.nombre ?? '—' }}</td>
            <td class="px-4 py-1.5">
              <span class="px-2 py-0.5 rounded-sm border text-xs font-medium" :style="estadoBadgeStyle(r.estado)">
                {{ r.estado }}
              </span>
            </td>
            <td class="px-4 py-1.5" :style="{ color: 'var(--sapContent_LabelColor)' }">{{ r.turno }}</td>
            ...
            <td class="px-4 py-1.5 text-center">{{ r.detalles?.length ?? '—' }}</td>
            <!-- Acciones según estado -->
            <td class="px-4 py-1.5">
              <div class="flex items-center gap-1">
                <!-- Pendiente -->
                <template v-if="r.estado === 'Pendiente'">
                  <button @click="aprobar(r.id)" class="h-6 px-2 text-xs rounded-sm border transition-colors"
                  ...
                  <button @click="abrirRechazar(r)" class="h-6 px-2 text-xs rounded-sm border transition-colors"
                ...
                <!-- Aprobada -->
                <router-link v-if="r.estado === 'Aprobada'" :to="`/app/reservas/despachar/${r.id}`"
                  class="h-6 px-2 text-xs rounded-sm font-medium transition-colors"
                  :style="{ background: 'var(--sapButton_Emphasized_Background)', color: 'var(--sapButton_Emphasized_TextColor)', border: 'none' }">
                  Despachar
                </router-link>
                <!-- Rechazada: mostrar motivo -->
                <span v-if="r.estado === 'Rechazada' && r.motivo_rechazo"
                  class="text-xs truncate max-w-[120px]" :style="{ color: 'var(--sapNegativeTextColor)' }"
          ...
          <tr v-if="!reservasStore.loading && !reservasStore.items.length">
            <td colspan="8" class="px-4 py-8 text-center text-xs" :style="{ color: 'var(--sapContent_LabelColor)' }">
    ...
    <!-- Dialog Rechazar -->
    <div v-if="dialogRechazar" class="fixed inset-0 z-50 flex items-center justify-center" style="background: rgba(0,0,0,0.4)">
      <div class="w-full max-w-md rounded-sm border shadow-xl"
        :style="{ background: 'var(--sapGroup_ContentBackground)', borderColor: 'var(--sapList_BorderColor)', boxShadow: 'var(--sapContent_Shadow2)' }">
        <div class="px-5 py-4 border-b flex items-center gap-2" :style="{ borderColor: 'var(--sapList_BorderColor)' }">
          <span class="sap-icon--decline" :style="{ color: 'var(--sapNegativeTextColor)' }" />
          <h2 class="text-base font-semibold" :style="{ color: 'var(--sapTextColor)' }">Rechazar Reserva</h2>
        </div>
        <div class="p-5 space-y-3">
          ...
          <div class="space-y-1">
            <label class="text-xs font-normal" :style="{ color: 'var(--sapContent_LabelColor)' }">Motivo de rechazo *:</label>
            <textarea v-model="motivoRechazo" rows="3" placeholder="Indica el motivo del rechazo..."
        ...
        <div class="flex gap-3 px-5 py-4 border-t" :style="{ borderColor: 'var(--sapList_BorderColor)' }">
          <button @click="dialogRechazar = false" class="flex-1 h-9 text-sm border rounded-sm"
          ...
          <button @click="confirmarRechazo" :disabled="!motivoRechazo.trim() || rechazando"
            ...
            :style="{ background: 'var(--sapButton_Reject_Background)', color: 'var(--sapButton_Reject_TextColor)', border: 'none' }">
            <span v-if="rechazando" class="sap-icon--refresh animate-spin mr-1" />
            Rechazar
          </button>
        </div>
      </div>
