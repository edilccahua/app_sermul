  ...
  <div class="h-full flex flex-col" :style="{ background: 'var(--sapBackgroundColor)' }">
    <div class="px-6 py-4 border-b shrink-0" :style="{ background: 'var(--sapPageHeader_Background)', borderColor: 'var(--sapList_BorderColor)' }">
      <div class="flex items-center gap-3">
        <span class="sap-icon--approvals text-2xl" :style="{ color: 'var(--sapButton_Emphasized_Background)' }" />
        <div>
          <h1 class="text-xl font-bold" :style="{ color: 'var(--sapTextColor)' }">Aprobar Reservas</h1>
          <p class="text-sm" :style="{ color: 'var(--sapContent_LabelColor)' }">Reservas pendientes de aprobación</p>
        </div>
        <span class="ml-auto text-sm px-2 py-0.5 rounded-full font-bold"
          :style="{ background: 'var(--sapErrorBackground)', color: 'var(--sapNegativeTextColor)' }"
          v-if="pendientes.length">{{ pendientes.length }}</span>
      </div>
    </div>
      ...
      <table v-else class="w-full text-xs border-collapse">
        <thead>
          <tr :style="{ background: 'var(--sapList_HeaderBackground)', color: 'var(--sapContent_LabelColor)' }">
            ...
            <th class="px-4 py-1.5 text-left font-semibold uppercase tracking-wide border-b" :style="{ borderColor: 'var(--sapList_BorderColor)' }">Grupo</th>
            ...
            <th class="px-4 py-1.5 text-left font-semibold uppercase tracking-wide border-b" :style="{ borderColor: 'var(--sapList_BorderColor)' }">Turno</th>
            <th class="px-4 py-1.5 text-left font-semibold uppercase tracking-wide border-b" :style="{ borderColor: 'var(--sapList_BorderColor)' }">Fecha</th>
            <th class="px-4 py-1.5 text-center font-semibold uppercase tracking-wide border-b" :style="{ borderColor: 'var(--sapList_BorderColor)' }">Ítems</th>
        ...
        <tbody>
          <tr v-for="r in pendientes" :key="r.id"
          ...
          <tr v-if="!pendientes.length">
            <td colspan="7" class="px-4 py-8 text-center text-xs" :style="{ color: 'var(--sapContent_LabelColor)' }">
              <span class="sap-icon--accept block text-3xl mb-2" :style="{ color: 'var(--sapPositiveTextColor)' }" />
    ...
    <div v-if="dialogRechazar" class="fixed inset-0 z-50 flex items-center justify-center" style="background: rgba(0,0,0,0.4)">
      <div class="w-full max-w-md rounded-sm border shadow-xl"
        ...
        <div class="p-5 space-y-3">
          ...
          <div class="space-y-1">
            <label class="text-xs font-normal" :style="{ color: 'var(--sapContent_LabelColor)' }">Motivo *:</label>
            <textarea v-model="motivoRechazo" rows="3" class="w-full border px-3 py-2 text-sm rounded-sm resize-none focus:outline-none"
