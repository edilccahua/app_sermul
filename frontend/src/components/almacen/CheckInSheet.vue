    ...
    <SheetContent side="right" class="w-[420px] sm:w-[500px] flex flex-col" @open-auto-focus.prevent>
      <SheetHeader class="pb-4 border-b border-border">
        <SheetTitle class="flex items-center gap-2">
          <ArrowLeftOnRectangleIcon class="w-5 h-5 text-emerald-400" />
      ...
      <div class="flex-1 overflow-y-auto py-5 space-y-5">
        ...
        <div class="space-y-1.5">
          ...
          <div class="relative">
            <ArrowLeftOnRectangleIcon class="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-muted-foreground" />
            <input
              ref="inputRef"
              v-model="query"
              placeholder="ej: HER-001 — busca unidades En_Uso"
              class="flex h-11 w-full rounded-md border border-input bg-background pl-9 pr-3 text-sm font-mono focus:outline-none focus:ring-2 focus:ring-emerald-500/50 transition-all"
        ...
        <div v-if="unidadesEnUso.length > 0" class="space-y-2">
          ...
          <div class="rounded-md border border-border overflow-hidden">
            ...
            >
              ...
              <span class="shrink-0 inline-flex items-center rounded-full px-2 py-0.5 text-xs font-medium bg-blue-500/15 text-blue-400 border border-blue-500/30 mt-0.5">
                En Uso
              </span>
            </button>
          </div>
        ...
        <div v-else-if="query.length >= 2 && !loading && unidadesEnUso.length === 0 && results.length === 0"
          class="rounded-lg border border-amber-500/30 bg-amber-500/10 p-4 text-sm text-amber-400">
        ...
        <div v-if="unidadSeleccionada" class="space-y-4">
          ...
          <div class="rounded-lg border border-border p-4 space-y-3">
            ...
            <div v-if="buenEstado === false" class="space-y-1.5">
              <label class="text-sm font-medium text-rose-400">Describe el daño *</label>
              <textarea
                v-model="descripcionDano"
                placeholder="ej: Piñón ratchet quebrado, cable pelado..."
                rows="3"
                class="flex w-full rounded-md border border-rose-500/50 bg-background px-3 py-2 text-sm focus:outline-none focus:ring-1 focus:ring-rose-500/50"
              />
            </div>
          </div>
        </div>
      ...
      <SheetFooter class="pt-4 border-t border-border gap-3">
        ...
        >
          <span v-if="confirmando" class="flex items-center justify-center gap-2">
            <ArrowPathIcon class="w-4 h-4 animate-spin" />Registrando...
          </span>
