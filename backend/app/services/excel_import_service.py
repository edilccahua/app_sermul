...
class ExcelImportService:
    ...
    def _get_rol_id(self, cargo: str) -> int:
        cargo = cargo.upper() if isinstance(cargo, str) else ""
    ...
    def importar_excel(self, file: bytes, parada_id: int):
        ...
        if group_headers:
            for gh in group_headers:
                ...
                lider_dni: str | None = None
                supervisor_dni: str | None = None
                ...
                for r in range(row_start, len(df_full)):
                    ...
                    if "SUPERVISOR DE OPERACION" in cargo_val.upper():
                        supervisor_dni = dni_str
                    elif "SUPERVISOR" not in cargo_val.upper() and not lider_dni:
                        lider_dni = dni_str
                        
                if not supervisor_dni and miembros:
                    supervisor_dni = miembros[0]["dni"]
                if not lider_dni and miembros:
                    lider_dni = miembros[0]["dni"]
                ...
                for m in miembros:
                    ...
                    if u:
                        ...
                        u = Usuario(
                            ...
                            nombre=nombres,
                            apellido=apellido,
                            password_hash=hash_password("sermul123"),
                    ...
                    miembros_ids.append((u.id, m["dni"], m["cargo"]))
                    
                    if m["dni"] == lider_dni:
                        lider_id = u.id
                    if m["dni"] == supervisor_dni:
                        supervisor_id = u.id
                ...
                g = self.db.query(GrupoTrabajo).filter(GrupoTrabajo.codigo == codigo_grupo, GrupoTrabajo.parada_id == parada_id).first()
                if g:
                    g.nombre = nombre_grupo
                    g.lider_id = lider_id
                    g.supervisor_id = supervisor_id
                    self.db.commit()
                ...
                for (uid, udni, ucargo) in miembros_ids:
                    self.db.query(GrupoIntegrante).filter(
                        GrupoIntegrante.usuario_id == uid,
                    ...
                    if existente:
                        ...
                        nuevo = GrupoIntegrante(grupo_id=g.id, usuario_id=uid, activo=True)
                        self.db.add(nuevo)
